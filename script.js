...
/* =====================================================
   Testimonial Swiper
===================================================== */
var swiper = new Swiper(".krish-silent-swiper", {
   slidesPerView: 1,
   spaceBetween: 30,
   loop: true,
   pagination: {
...
(function() {
   ...
   emailjs.init({
     publicKey: "j0p2oAyH7gU3opyKv",
   });
})();

krishContactForm = document.getElementById("krish-contact-form");
krishContactFormAlert = document.querySelector(".contact-form-alert");

krishContactForm.addEventListener('submit', function(event) {
   event.preventDefault();
   // these IDs from the previous steps
   emailjs.sendForm('service_2tyw4tc', 'template_fekihbi', '#krish-contact-form')
       .then(() => {
         //   console.log('SUCCESS!');
         krishContactFormAlert.innerHTML = "<span>Your message sent successfully!</span> <i class='ri-checkbox-circle-fill'></i>";
         krishContactForm.reset();

         setTimeout(() => {
            krishContactFormAlert.innerHTML = "";
         }, 5000);

       }, (error) => {
         //   console.log('FAILED...', error);
         krishContactFormAlert.innerHTML = "<span>Message not sent</span> <i class='ri-error-warning-fill'></i>";
         krishContactFormAlert.title = error;
       });
});
/* =====================================================
...
window.addEventListener("scroll", () => {
   const krishHeader = document.querySelector(".krish-header");

   krishHeader.classList.toggle("shrink", window.scrollY > 0);
});
/* =====================================================
...
const cursorHoverlinks = document.querySelectorAll("body a, .theme-btn, .krish-main-btn, .portfolio-card, .swiper-button-next, .swiper-button-prev, .swiper-pagination-bullet, .service-card, .contact-social-links li, .contact-form .submit-btn, .menu-show-btn, .menu-hide-btn");
...
themeBtn.addEventListener("click", () => {
   ...
   localStorage.setItem("krish-saved-icon", getCurrentIcon());
   localStorage.setItem("krish-saved-theme", getCurrentTheme());
   
});
// Get saved theme icon and theme on document loaded.
const savedIcon = localStorage.getItem("krish-saved-icon");
const savedTheme = localStorage.getItem("krish-saved-theme");
...
ScrollReveal().reveal('.about-info, krish-footer .krish-logo', {delay: 300, origin: 'bottom' });
ScrollReveal().reveal('.pro-card, .about-buttons .krish-main-btn .resume-tabs .tab-btn, .portfolio-tabs .tab-btn', {delay: 300, origin: 'right', interval: 200 });
...
ScrollReveal().reveal('krish-client-swiper, .contact-form-body', {delay: 700, origin: 'right' });
